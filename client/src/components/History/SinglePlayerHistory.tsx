...
export const SinglePlayerHistory: React.FC<SinglePlayerHistoryProps> = ({
  user_id,
}) => {
  const [singleHistory, setSingleHistory] = useState<
    Array<SinglePlayHistoryType>
  >([]);
    ...
    <>
      {loading ? (
        ...
        <div className="grid grid-cols-1 gap-4">
          {singleHistory.map(
            (history: SinglePlayHistoryType, index: number) => (
              <Link
                to={`/history/singleplay/${history.id}`}
                key={history.id}
              ...
              >
                ...
                >
                  <div className="flex items-center space-x-4">
                    <Star
                      className={`h-8 w-8 ${
                        categories.includes(history.category)
                          ? "text-[#4ECDC4]"
                          : "text-[#FF6B6B]"
                      }`}
                    />
                    <div>
                      <p className="font-bold md:text-wrap text-nowrap">
                        {history.category == "" ? "ランダム" : history.category}
                      </p>
                      <p className="text-sm text-gray-600">
                        {formatDateToMonthDay(history.created_at)}
                      </p>
                    </div>
                  </div>
                  <div className="flex items-center space-x-4">
