...
export const MultiPlayerHistory: React.FC<MultiPlayerHistoryProps> = ({
  user_id,
}) => {
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 gap-4">
          {multiHistory.map((battle: MultiPlayHistoryType, index: number) => (
            <Link
              to={`/history/multiplay/${battle.session_id}`}
              key={battle.session_id}
                ...
                <div className="flex items-center space-x-4">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-gray-600">
                      {formatDateToMonthDay(battle.created_at)}
                    </p>
                  </div>
                </div>
                <div className="flex items-center space-x-4">
                  <div className="text-right">
                    <p className="font-semibold hidden md:block">
                      +{battle.points_awarded}ポイント
                    </p>
                    <p className="text-sm text-gray-600">
                      {battle.who_win === user_id ? "勝利" : "敗北"}
                    </p>
                  </div>
