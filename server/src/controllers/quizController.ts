...
export const generateQuiz = async (category?: string, difficulty?: string, user_id?: number, opponent_id?: number) => {
    const categories = ["科学", "日本史", "文学", "地理", "世界史", "情報", "一般常識"];
    ...
    let opponentQuestionsString: any;
    if (opponent_id) {
        ...
        opponentQuestionsString = JSON.stringify(opponentQuestions);
    }
            ...
            messages: [
                {
                    ...
                    **過去に生成した問題や類似の表現、または同じテーマを使わないように注意してください。**
                    ...
                    また、問題の出題にあたっては、**バラエティを持たせるためにテーマや内容が異なるものを優先して生成してください**。
