...
export type GenerateQuizResponse =
  | QuizType
  | { error: string; details?: any; response?: any };
...
export const generateQuiz = async (
  ...
  if (category && category === "科学") {
    subcategory =
      scienceCategories[Math.floor(Math.random() * scienceCategories.length)];
  }
  if (category && category === "地理") {
    subcategory =
      geographyCategories[
        Math.floor(Math.random() * geographyCategories.length)
      ];
  }
  ...
  if (category && category === "情報") {
    subcategory =
      informationCategories[
        Math.floor(Math.random() * informationCategories.length)
      ];
  }
  if (category && category === "一般常識") {
    subcategory =
      generalKnowledgeCategories[
        Math.floor(Math.random() * generalKnowledgeCategories.length)
      ];
  }
  if (category && category === "文学") {
    subcategory =
      literatureCategories[
        Math.floor(Math.random() * literatureCategories.length)
      ];
  }
  if (category && category === "工学") {
    subcategory =
      engineeringCategories[
        Math.floor(Math.random() * engineeringCategories.length)
      ];
  }
  if (category && category === "環境") {
    subcategory =
      environmentCategories[
        Math.floor(Math.random() * environmentCategories.length)
      ];
  }
    ...
    while (isDuplicate) {
      ...
      try {
        ...
        if (parsedQuiz !== undefined && pastQuestionsSet !== undefined) {
          isDuplicate = await isSimilarQuestion(parsedQuiz.question, [
            ...pastQuestionsSet,
          ]);
          if (isDuplicate) {
